...
clean:
	...
	rm -rf _minted-*
distclean: clean
	rm -f $(NAME).{pdf,ins} $(NAME)-slides.pdf $(NAME).cls rdacolors.sty beamerthemeRDA.sty beamerthemeRDA2016.sty rdams{c,d}wg.sty $(NAME)-sample-{RDA,RDA2016}.{tex,pdf}
inst: all
	mkdir -p $(UTREE)/{tex,source,doc}/latex/$(NAME)
	mkdir -p $(UTREE)/tex/generic/logos-rda
	cp $(NAME).dtx $(NAME).ins $(UTREE)/source/latex/$(NAME)
	cp $(NAME).cls rdacolors.sty beamerthemeRDA.sty beamerthemeRDA2016.sty rdams{c,d}wg.sty rda-bg-normal.jpeg rda-bg-title1.jpeg rda-bg-title2.jpeg $(UTREE)/tex/latex/$(NAME)
...
install: all
	...
	sudo cp $(NAME).cls rdacolors.sty beamerthemeRDA.sty beamerthemeRDA2016.sty rdams{c,d}wg.sty rda-bg-normal.jpeg rda-bg-title1.jpeg rda-bg-title2.jpeg $(LOCAL)/tex/latex/$(NAME)
...
zip: all
	mkdir $(TDIR)
	cp $(NAME).{pdf,dtx} $(NAME)-slides.pdf $(NAME).cls rdacolors.sty beamerthemeRDA.sty beamerthemeRDA2016.sty rdams{c,d}wg.sty $(NAME)-sample-{RDA,RDA2016}.{tex,pdf} README.md Makefile rda-bg-normal.jpeg rda-bg-title1.jpeg rda-bg-title2.jpeg rda-logo.{eps,pdf} $(TDIR)
